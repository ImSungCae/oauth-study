...
import com.oauthstudy.api.entity.user.UserRefreshToken;
...
import com.oauthstudy.oauth.info.OAuth2UserInfo;
import com.oauthstudy.oauth.info.OAuth2UserInfoFactory;
import com.oauthstudy.oauth.repository.OAuth2AuthorizationRequestBasedOnCookieRepository;
import com.oauthstudy.oauth.token.AuthToken;
import com.oauthstudy.oauth.token.AuthTokenProvider;
import com.oauthstudy.utils.CookieUtil;
...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    private boolean isAuthorizedRedirectUri(String uri){
        URI clientRedircetUri = URI.create(uri);

        return appProperties.getOauth2().getAuthorizedRedirectUris()
                .stream()
                .anyMatch(authorizedRedirectUri -> {
                    URI authorizedURI = URI.create(authorizedRedirectUri);
                    if(authorizedURI.getHost().equalsIgnoreCase(clientRedircetUri.getHost())
                        && authorizedURI.getPort() == clientRedircetUri.getPort()){
                        return true;
                    }
                    return false;
                });
